...
- Main b651583 is integrated; the macOS fixture uses T053 consumer identity org.nativeui.test.macos-drop rather than a manual runtime prefix. Integrated head caaf4c0 passed all four remote CI jobs and T053 two-consumer isolation. Prior image-crash head f5d29fc passed macOS/Linux CI, but Windows timed out only in the unchanged T047 CMake attach-contract test; font/drop/T018 passed there. Do not report that matrix as fully green.
...
- Cleanup TDD: a valid-text `.jpg` fixture failed under the allow-list, then passed after removal. The same contents now pass as `.txt`, `.md`, `.jpg` and extensionless files. Shared UTF-8 prefix checks cover all 256 single bytes, malformed sequences, borrowed lifetime and byte-limit boundaries. Final local Release CTest is 63/63 PASS and targeted font/drop/example/native ASan+UBSan is 4/4 PASS, with no compiler warnings. Corrective-head CI is still required.
...
Next action for this lane: verify the extension-independent corrective head's CI and real Finder binary-to-text smoke, then request merge direction. The three-pass/CODE_REVIEW.md record in #87 covers this cleanup; other lanes/frontiers above remain unchanged.
