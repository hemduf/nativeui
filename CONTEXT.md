...
Finder drops exposed a second integration defect after the Pugl backend-child destination fix: NativeUI discarded DropOffer/DropData whenever the window lost keyboard focus. The retained tree now routes drops while mounted but inactive, without reactivating focus or IME, preserving T059 availability and dispatch reconciliation. The T018 example reads the first local regular UTF-8 `.txt` file from a URI list (percent-decoding, 64 KiB cap, 120-byte preview); both limits preserve complete characters. `--trace-drops` logs only acceptance and byte count.

The user's subsequent image drop reached painting but crashed in macOS Skia text-to-glyph conversion: fallback decoded U+FFFD but still measured/rendered the original invalid bytes. The shared text layout now owns repaired UTF-8 only when needed; T018 also rejects images and disguised binary `.txt` files without painting their bytes.
...
- Main b651583 is integrated; the macOS fixture uses T053 consumer identity org.nativeui.test.macos-drop rather than a manual runtime prefix. Integrated head caaf4c0 passed all four remote CI jobs and T053 two-consumer isolation. The image-crash correction passes the local Release 63-test suite plus four ASan+UBSan font/drop/example/native tests, with no compiler warnings; its own exact-head CI remains pending.
...
Next action for this lane: verify the image-crash corrective head's CI and real Finder image-to-text smoke, then request merge direction. The three-pass review and CODE_REVIEW.md evidence are recorded in #87; other lanes/frontiers above remain unchanged.
