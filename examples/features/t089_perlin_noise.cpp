...
ui::Brush wood(const ui::NoiseSource& source) {
    return source.as_brush({0.08f, 0.025f, 0.01f, 1.0f},
                           {0.32f, 0.10f, 0.02f, 1.0f});
}
...
int self_test() {
    ...
    const auto other = ui::NoiseSource::create(
        ui::NoiseType::Perlin, {.feature_size = 72.0f, .seed = 0x27182818u});
    if (!first.ok() || !repeat.ok() || !other.ok()) {
        return example::fail("built-in perlin-noise source failed to compile");
    }
    ...
    const auto [ok_a, a] = render_field(wood(first.noise));
    const auto [ok_b, b] = render_field(wood(repeat.noise));
    const auto [ok_c, c] = render_field(wood(other.noise));
    if (!ok_a || !ok_b || !ok_c) return example::fail("noise render failed");
    ...
    auto scene = make_scene(wood(first.noise), wood(other.noise));
    ui::HeadlessRenderer renderer{{320, 180}, 1.0f};
...
int main(int argc, char** argv) {
    ...
    if (!first.ok() || !second.ok()) {
        std::cerr << "Perlin noise SkSL failed: "
                  << (first.ok() ? second.diagnostic : first.diagnostic) << '\n';
        return 1;
    }
    auto tree = make_scene(wood(first.noise), wood(second.noise));
    return example::run_window(tree, "NativeUI T089 perlin noise", {320, 180});
}
