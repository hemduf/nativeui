...
ui::Rect step_rect(ui::Size size, int index) {
    ...
    return {kSequencerPadding + static_cast<float>(index) * (width + kStepGap),
            kStepY, width, kStepHeight};
}
...
int step_at(ui::Size size, ui::Point point) {
    ...
    return step_rect(size, index).contains(point) ? index : -1;
}
        ...
        ui::Column{
            ...
            ui::Canvas{
                ...
                [&](ui::CanvasContext2D& g) {
                    ...
                    g.fill_rounded_rect({0.0f, 0.0f, size.w, size.h}, 14.0f, ui::colors::panel);
                    g.stroke_rounded_rect({0.0f, 0.0f, size.w, size.h}, 14.0f,
                        g.focused() ? 2.0f : 1.0f,
                        g.focused() ? ui::colors::borderFocus : ui::colors::border);
                    g.text({kSequencerPadding, 22.0f}, "STEP SEQUENCER", 13.0f,
                           ui::colors::text, ui::TextAlign::Left);
                    g.text({size.w - kSequencerPadding, 22.0f},
                           "click/drag  |  arrows  |  space  |  delete", 10.0f,
                           ui::colors::textMuted, ui::TextAlign::Right);
                    ...
                    for (int i = 0; i < kStepCount; ++i) {
                        ...
                        g.fill_rounded_rect(rect, 7.0f,
                            active ? ui::colors::accent : ui::colors::knobInner);
                        const auto outline = selected && g.focused()
                            ? ui::colors::text : ui::colors::border;
                        g.stroke_rounded_rect(rect, 7.0f,
                            selected && g.focused() ? 2.0f : 1.0f, outline);
                        g.text({rect.x + rect.w * 0.5f, rect.y + rect.h * 0.5f},
                               std::to_string(i + 1), 10.0f,
                               active ? ui::colors::background : ui::colors::textMuted,
                               ui::TextAlign::Center);
                    }
                    g.text({kSequencerPadding, size.h - 17.0f},
                           "Mouse: click or drag to add/remove   Keyboard: left/right, space, up=add, down/delete=remove",
                           10.0f, ui::colors::textMuted, ui::TextAlign::Left);
                }}
                .on_input([&](const ui::InputEvent& event, ui::CanvasInputContext& ctx) {
                    ...
                    case ui::Key::Right:
                        ...
                        break;
                    case ui::Key::Home: sequencer.cursor = 0; break;
                    case ui::Key::End: sequencer.cursor = kStepCount - 1; break;
                    case ui::Key::Space:
                    case ui::Key::Enter: {
