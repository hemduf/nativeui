...
int main() {
    ...
    const auto path_a = transport.register_object_path(client_a, "/org/nativeui/T072/ClientA",
        [](const LinuxDbusMethodRequest&) {
            return LinuxDbusMethodReply::method_return({LinuxDbusValue::string("a")});
        });
    const auto path_b = transport.register_object_path(client_b, "/org/nativeui/T072/ClientB",
        [](const LinuxDbusMethodRequest&) {
    ...
    const auto request = transport.call_method(
        ...
        LinuxDbusMethodCall{transport.unique_name(), "/org/nativeui/T072/ClientB",
                            "org.nativeui.T072.Test", "Ping", 2s},
        [&](LinuxDbusCompletion result) { reply = std::move(result); reply_done = true; });
