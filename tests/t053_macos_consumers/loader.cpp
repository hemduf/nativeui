...
int main(int argc, char** argv) {
    if (argc != 5) {
        std::cerr << "usage: t053_consumer_loader <module-a> <prefix-a> <module-b> <prefix-b>\n";
        return 2;
    }

    void* const module_a = dlopen(argv[1], RTLD_NOW | RTLD_LOCAL);
    if (!module_a) {
        std::cerr << "failed to load first T053 consumer: " << dlerror() << '\n';
        return 3;
    }
    void* const module_b = dlopen(argv[3], RTLD_NOW | RTLD_LOCAL);
    if (!module_b) {
        std::cerr << "failed to load second T053 consumer: " << dlerror() << '\n';
        return 4;
    }
    ...
    // Do not dlclose Objective-C class-bearing images: the runtime namespace is
    // process-global and unloading class implementations is not a lifecycle
    // contract NativeUI relies on. Process exit provides the fixture teardown.
    (void)module_a;
    (void)module_b;
    return 0;
}
