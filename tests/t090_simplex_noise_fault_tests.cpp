...
void fault_and_recovery() {
    ...
    const auto failed = ui::NoiseSource::create(ui::NoiseType::Simplex);
    ...
    const auto recovered = ui::NoiseSource::create(ui::NoiseType::Simplex);
    ...
    bool threw = false;
    try {
        static_cast<void>(ui::NoiseSource::create(ui::NoiseType::Simplex));
    ...
    const auto after_allocation = ui::NoiseSource::create(ui::NoiseType::Simplex);
    check(after_allocation.ok(),
          "creation after allocation failure did not recover");
...
int main() {
    try {
        ...
        std::cerr << "FAIL T090 simplex noise fault: " << e.what() << '\n';
        return 1;
    }
}
