...
void fault_and_recovery() {
    ...
    const auto failed = ui::NoiseSource::create(ui::NoiseType::Value);
    ...
    const auto recovered = ui::NoiseSource::create(ui::NoiseType::Value);
    ...
    bool threw = false;
    try {
        static_cast<void>(ui::NoiseSource::create(ui::NoiseType::Value));
    ...
    const auto after_allocation = ui::NoiseSource::create(ui::NoiseType::Value);
...
int main() {
    try {
        ...
        std::cerr << "FAIL T088 noise fault: " << e.what() << '\n';
        return 1;
    }
}
