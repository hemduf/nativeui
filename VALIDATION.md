...
- The refreshed candidate also updates `THIRD_PARTY.md`, `ROADMAP.md` and this validation record as required by the dependency-update workflow. The recorded validation includes normal CI, the historical T042 Lifecycle Stress run and a clean mandatory `CODE_REVIEW.md` pass.

## T177 raw multi-touch pointer integration — 2026-09-23

- NativeUI pins reachable `hemduf/pugl` commit `94982803985eefcbaeb0a1c8d0136ec862d7cb59`, which provides stable raw pointer IDs, source type, pressure, contact geometry and sample flags. The macOS Cocoa bridge sources consumed by NativeUI are unchanged between the previous pin and this revision; the T053 consumer-specific runtime prefix remains in force.
- The NativeUI public `InputEvent` appends backend-neutral pointer metadata without changing existing input enum values or aggregate-field order. Pugl translation stays in the platform adapter; legacy mouse input remains ID 0.
- RED regressions reproduced cross-ID release through an outer `InputContext` during nested dispatch, same-ID stale capture during nested dispatch, terminal `PointerUp` capture renewal and failed `PointerDown` cleanup after nested capture renewal. GREEN tests prove later normal routing/recovery and two-Tree survivor behavior.
- Local macOS arm64 Release configure used the exact Pugl source checkout and pinned `chrome/m153` Skia binary. `cmake --build /private/tmp/nativeui-t177-build` completed serially with default empty `NATIVEUI_ALLOWED_WARNINGS` and no warnings. Targeted multi-touch, Pugl translation, legacy pointer-capture and `nativeui_example_t177_multitouch_pointer --self-test` checks passed.
- Full local CTest registered 173 tests: 168 passed in the sandbox; the five known AppKit window tests failed at sandboxed `NSWindow` creation with a non-finite frame, then all five passed when rerun in the graphical session. Effective local result: **173/173 PASS**. Exact-head normal/path and release qualification is recorded in PR #443.
