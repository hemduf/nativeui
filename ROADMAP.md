...
- **T096 / #180 (in progress):** localized paint damage now has a bounded capture/rollback transaction, conservative physical clip mapping, a clipped overwrite reset and T094 culling into the persistent per-view scene. Layout changes, overlays, inspector and unproven filtered-effect cases retain the full repaint path. Partial-scene GPU parity/failure coverage and deterministic Tree transaction recovery coverage are implemented. The local macOS Release build and CTest results are recorded in the active implementation review; native GPU execution still needs a display-capable macOS run before merge.
