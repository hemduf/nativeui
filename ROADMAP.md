...
- **T087 / #171 / PR #451 — review candidate:** `ImageTexture` adds `TextureInterpretation::{Color,Data}` with Color as the default and interpretation stored strictly per texture value. `Image::decode` performs one encoded decode into an eager unpremultiplied numeric raster, then derives an immutable premultiplied sibling from those already-decoded pixels so Color keeps coverage-correct filtering/compositing while Data retains raw RGB independently from alpha. Data materialization uses pinned-Skia raw image shaders, ignores embedded RGB transfer/gamut metadata, keeps alpha numeric, and avoids `SkPictureShader` because that path rasterizes into premultiplied sRGB. Shared Images therefore support independent Color/Data views without re-decode; tile/filter/mipmap/transform state remains composable and T097 remains the owner of retained materialization caching. Dedicated coverage includes tagged and untagged color, alpha payloads, premultiplied-filter regression, normal-like data, shared-Image isolation, transform snapshots, tiling/filter/mipmap combinations, failure recovery and the feature example.

## Release policy

T069/#81 is deprecated and no longer acts as a standalone v1 freeze gate. The current backend-neutral public headers/package contracts plus exact-head T070/T122/T071 validation are authoritative for v1 closeout. T073, T074, T075, T076, T077, T079, T080, T081, T082, T083, T084, T085, T086 and T098 are merged as later-release rendering foundations outside the v1 critical path. T087 / #171 / PR #451 is under review on that later-release rendering line. T078 remains Ready on the effects line. Retained per-view shader/resource caching remains deferred to T097. T070 validates the reference application against the frozen surface. T071 is validation/release-only: defects found there return to a focused canonical ticket, are merged first, and then a new exact release-candidate SHA is selected. T068 remains outside the v1 critical path and is targeted for NativeUI 1.2.